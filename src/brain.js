...
const MINECRAFT_API = 'http://127.0.0.1:8765';

let minecraftRunning = false;
let minecraftLoop = null;
...
async function minecraftRequest(path, options = {}) {
  ...
  try {
    const res = await fetch(`${MINECRAFT_API}${path}`, {
    ...
    if (!res.ok || data.ok === false) {
      throw new Error(data.error || 'Minecraft API error');
    }
...
export async function getMinecraftState() {
  return minecraftRequest('/state', {
    timeout: 5000,
  });
}

async function minecraftAction(action) {
  return minecraftRequest('/action', {
    method: 'POST',
    body: action,
    timeout: 90000,
  });
...
async function minecraftSay(text) {
  ...
  try {
    ...
    console.warn('[Minecraft chat]', e.message);
  }
}

export async function stopAutoplay() {
  minecraftRunning = false;

  if (minecraftLoop) {
    clearTimeout(minecraftLoop);
    minecraftLoop = null;
  }

  try {
    await minecraftAction({
      type: 'stop',
    });
  } catch {}
...
function minecraftSystemPrompt(state) {
  return `
Ты Юки — AI-игрок в Minecraft Java 1.21.1.

Ты реально играешь вместе с человеком.
Ты НЕ управляешь клавиатурой и мышью.
Ты НЕ делаешь скриншоты Minecraft.
Ты НЕ используешь press_key, WASD, move_mouse или click_screen.

У тебя есть Minecraft-агент Mineflayer, который сам умеет:
- ходить
- искать игроков
...
Твоя задача — быть МОЗГОМ.
Принимай решения на основании состояния Minecraft.

ВАЖНО:
- Не придумывай предметы, которых нет в состоянии.
- Не говори, что действие выполнено, пока executor не вернул успех.
- Не делай бессмысленные действия.
- Не повторяй одну и ту же неудачную команду бесконечно.
- Если рядом игрок — учитывай его.
- Ты можешь работать самостоятельно, но мы играем вместе.
- При опасности сначала спасай себя.
- Если мало еды — добывай/ешь еду.
- Если мало HP — избегай боя и восстанавливайся.
- Если рядом creeper — не подходи к нему.
- Ночью учитывай необходимость укрытия/кровати.
- Всегда имей долгосрочную цель.

ПРОГРЕССИЯ:

1. Дерево
2. Верстак
3. Каменные инструменты
4. Еда
5. Железо
6. Щит
7. Железная броня
8. Алмазы
9. Алмазные инструменты
10. Обсидиан
11. Nether
12. Blaze rods
13. Ender pearls
...
Если пользователь дал конкретную цель — она важнее общей прогрессии.
...

{
  "say": "короткая фраза",
  "emotion": "happy|neutral|surprised|sad|angry|relaxed",
  "goal": "текущая долгосрочная цель",
  "actions": [
    {
      "type": "mine",
      "block": "oak_log",
...
function normalizeMinecraftPlan(raw) {
  ...
  if (start === -1 || end === -1 || end <= start) {
    throw new Error('LLM не вернул Minecraft JSON');
  }
...
function minecraftEmergencyActions(state) {
  const actions = [];
  ...
  // Очень мало HP — не продолжаем обычную работу.
  if (Number(bot?.health) <= 5) {
    if (state.inventory?.some?.(i =>
      /food|apple|bread|beef|porkchop|chicken|mutton|carrot|potato/i.test(i.name)
    )) {
      actions.push({
        type: 'eat',
      });
  ...
  // Очень мало еды.
  if (Number(bot?.food) <= 4) {
    const food = state.inventory?.find(i =>
      /bread|apple|beef|porkchop|chicken|mutton|carrot|potato/i.test(i.name)
    );
  ...
  // Creeper слишком близко.
  const creeper = (state.nearbyEntities || []).find(e =>
    /creeper/i.test(e.name) &&
    Number(e.distance) < 7
  );
...
async function minecraftThink(state) {
  const emergency = minecraftEmergencyActions(state);

  if (emergency.length) {
    return {
      say: 'Опа. Сначала выживаем, потом геройствуем.',
      emotion: 'surprised',
      goal: 'survive',
      actions: emergency,
    };
  ...
  const msg = await llm(
    [
      {
        role: 'system',
        content: minecraftSystemPrompt(state),
      },
      {
        role: 'user',
        content: `
Мы сейчас играем в Minecraft.

Фокус пользователя:
${minecraftFocus || 'играй разумно и развивайся'}

Проанализируй состояние.
Выбери ближайшую полезную цель.
Дай максимум 3 действия.

Не делай огромный план на 30 действий.
После выполнения этих действий состояние будет обновлено,
и ты примешь следующее решение.
`,
      },
    ],
    {
      max_tokens: 500,
      temperature: 0.2,
      model: cfg.agentModel || cfg.textModel,
    }
  );

  return normalizeMinecraftPlan(msg);
}

async function executeMinecraftPlan(plan) {
  const actions = Array.isArray(plan.actions)
  ...
  for (const action of actions) {
    if (!minecraftRunning) break;

    phase('working');

    try {
      const result = await minecraftAction(action);
      ...
      if (action.type === 'follow_player') {
        // Follow — постоянная цель, не надо тут ждать завершения.
        break;
      }
    } catch (e) {
      console.warn(
      ...
      // Если действие не прошло — не спамим остальные действия.
      break;
    }
  }
...
async function minecraftLoopTick() {
  if (!minecraftRunning || minecraftBusy) return;

  minecraftBusy = true;

  try {
    const stateResponse = await getMinecraftState();

    if (!stateResponse.connected) {
      onAutoplaySay?.('Я не подключена к Minecraft.');
      await stopAutoplay();
      return;
    }
    ...
    // Состояние можно сохранять в память,
    // но не надо пихать его целиком в обычную память пользователя.
    console.log(
      '[Minecraft state]',
      `HP=${state.bot?.health}`,
      `Food=${state.bot?.food}`,
      `Pos=${JSON.stringify(state.bot?.position)}`,
      `Task=${state.task?.name || '-'}`
    );

    const plan = await minecraftThink(state);
    ...
    if (plan.say) {
      onAutoplaySay?.(plan.say);
    }

    if (plan.actions.length) {
      await executeMinecraftPlan(plan);
    }
  } catch (e) {
    console.warn('[Minecraft loop]', e.message);

    onAutoplaySay?.(
      `Что-то пошло не так: ${cleanSpeech(e.message)}`
    );
  } finally {
    minecraftBusy = false;
    phase('idle');
    ...
    if (minecraftRunning) {
      // Не 16Hz и не скриншоты.
      // Мозг принимает решение примерно раз в 4 секунды.
      minecraftLoop = setTimeout(
        minecraftLoopTick,
        4000
      );
    }
...
export async function startAutoplay(focusText = '') {
  if (minecraftRunning) {
    return;
  }

  const state = await getMinecraftState();

  if (!state.connected) {
    throw new Error(
      'Minecraft не подключен. Сначала запусти bot.js.'
    );
  }

  minecraftFocus = String(focusText || '');
  ...
  onAutoplaySay?.(
    'Всё, я в Minecraft. Теперь играю нормально, без этих долбаных скриншотов.'
  );

  minecraftLoopTick().catch(e => {
    console.error('[Minecraft start]', e);
  });
}
...
function detectAutoplayCommand(userText) {
  ...
  if (
    minecraftRunning &&
    /^(стоп|хватит|перестань|остановись|не играй|прекрати)/i.test(low)
  ) {
    return {
      mode: 'stop',
    };
  ...
  if (
    /minecraft|майнкрафт|в майнкрафт|в minecraft/i.test(low) &&
    /играй|игра|добы|строй|крафт|найди|собери|иди/i.test(low)
  ) {
    return {
      mode: 'start',
      focus: t,
...
export async function chat(userMessage, memory) {
  ...
  if (autoplayCmd) {
    ...
    if (autoplayCmd.mode === 'start') {
      if (isAutoplayActive()) {
        return {
          text: 'Я уже играю в Minecraft~',
          emotion: 'happy',
        };
      }

      try {
        await startAutoplay(autoplayCmd.focus);

        return {
          text: 'Окей. Я подключена к Minecraft и начинаю играть.',
          emotion: 'happy',
        };
      } catch (e) {
        console.warn('[Minecraft autoplay start]', e);

        return {
          text: `Не могу начать Minecraft: ${cleanSpeech(e.message)}`,
          emotion: 'sad',
        };
      }
    }
  ...
  // ============================================================
  // ПРЯМАЯ КОМАНДА: ЮКИ ИДИ КО МНЕ
  // ============================================================

  if (/юки\s+иди\s+ко\s+мне|yuki\s+come\s+here/i.test(text)) {
    try {
      const state = await getMinecraftState();

      if (!state?.connected) {
        return {
          text: 'Я сейчас не подключена к Minecraft.',
          emotion: 'sad',
        };
      }
      ...
      const player =
        state.nearbyPlayers?.find(
          (p) => p.name !== state.bot?.name
        ) || state.you;

      if (!player) {
        return {
          text: 'Я тебя пока не вижу.',
          emotion: 'sad',
        };
      }
      ...
      return {
        text: `Иду за тобой, ${player.name}~`,
        emotion: 'happy',
      };
    } catch (e) {
      return {
        text: `Не получилось: ${cleanSpeech(e.message)}`,
        emotion: 'sad',
      };
    }
  }
  ...
  try {
    // ==========================================================
    // PENDING CONFIRMATION — ПЕЧАТЬ ТЕКСТА
    // ==========================================================
    ...
    if (pendingConfirm?.kind === 'type_text') {
      ...
      if (isYes(text)) {
        ...
        return {
          text: r.ok
            ? 'Напечатала в нужное окно~'
            : `Не вышло: ${cleanSpeech(r.message)}`,
          emotion: r.ok ? 'happy' : 'sad',
        };
      }

      return {
        text: 'Жду да или нет на печать.',
        emotion: 'neutral',
      };
    }
    ...
    const heuristic = buildTaskPlan(
      text,
      memory
    );

    if (
      (!plan.actions || !plan.actions.length) &&
      heuristic?.actions?.length
    ) {
      plan = {
        remember: [],
        likes: [],
    ...
    // ==========================================================
    // FILE / SCREEN ACTION FIXES
    // ==========================================================

    plan = await ensureMakeActions(
      text,
      plan
    );
    ...
    if (stepCount > 1) {
      say =
        cleanSpeech(
          say ||
          `Ок, ${stepCount} шага — поехали.`
        ) ||
        `Делаю ${stepCount} шага.`;
    }
    ...
    if (plan.actions.length) {
      ...
      const madeFolder =
        (results || []).some(
          (r) =>
            /создала папку|открыла папку/i.test(
              String(r)
            )
        );
      ...
      const clicked =
        (results || []).some(
          (r) =>
            /кликн|открыла ссыл|открыла из|яндекс|ищу/i.test(
              String(r)
            )
        );
      ...
      if (
        ...
        say = failed
          ? 'Часть шагов не вышла — повтори.'
          : 'Готово по плану: папка и файл, открыла.';
      }
      ...
      // --------------------------------------------------------
        ...
        if (clicked) {
          ...
          say =
            cleanSpeech(
              `Вижу: ${desc}. Не смогла открыть — подвинь окно.`
            );
        }
      ...
      // --------------------------------------------------------
        ...
        say =
          'Блин, не все шаги прошли. Повтори коротко.';
      }
      ...
      // --------------------------------------------------------
        ...
        !say ||
        looksLikeRefusal(say)
      ) {
        say =
          stepCount > 1
            ? `Сделала ${stepCount} шага~`
            : 'Сделала~';
      }
    }
    ...
    if (looksLikeRefusal(say)) {
      say =
        'Ок, без вредности — повтори коротко что сделать.';
    }
